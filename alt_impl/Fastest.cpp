...
private:
    ...
    void add(PathSet& L, int s, int a, const Edge& e) {
        Iter it = find(L, a);
        if (it != L.begin() && prev(it)->s >= s) return; // Dominated
        if (it != L.begin() && prev(it)->a == a) --it;
        Iter j = it;
        while (j != L.end() && j->s <= s) ++j;
        L.insert(L.erase(it, j), Path(s, a, e));
    }
...
public:
    void compute(int n, vector<Edge>& edges, int source, int ta, int tw, vector<int>& t_arrival) {
        ...
        for (const Edge& e : edges) {
            ...
            int s = prev(it)->s, a = e.t + e.lambda;
            add(L[e.v], s, a, e);

            if (a - s < t_arrival[e.v]) {
                t_arrival[e.v] = a - s;
                arg_a[e.v] = a;
            }
        }
        ...
        for (int i = 1; i <= n; ++i)
            if (t_arrival[i] == INF) t_arrival[i] = -1;
    }
};
