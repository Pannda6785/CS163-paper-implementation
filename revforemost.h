...
public:
    ReverseForemost(const int &n, const vector<Edge> &edges, int ta, int tw, int target) : target(target), time(n + 1), parent(n + 1) {
        ...
        for (int edge_id = (int)edges.size() - 1; edge_id >= 0; edge_id--) {
            ...
            if (e.t < ta) break; // further edges in streaming presentation won't be used
    ...
    std::pair<int, vector<Edge>> getForemostPath(int source) {
        if (time[source] == -INF) { 
            // raise no path
    ...
    vector<int> getAllReverseForemostTime() {
        return time;
    }
};